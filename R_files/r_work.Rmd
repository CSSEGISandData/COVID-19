...
confirm <- read_csv("C:/Users/Morganak/Documents/R/COVID_Project/COVID-19/csse_covid_19_data/csse_covid_19_daily_reports/03-29-2020.csv")
names(confirm)
head(confirm)
```
...
md<-confirm %>% filter(Country_Region=="US" & Province_State=="Maryland")
head(md)
```
...
p3<-ggplot() +
  ...
  ylim(23,50) + xlim(-125,-60) + #resizing to just CONUS
  geom_point(data=us_only, aes(x=Long, y=Lat, alpha=0.5, color=Confirmed, size=Confirmed)) + #everything inside aes appears in legend
  coord_map() + #plotting with correct mercator projection (prior plot was cartesian coordinates)
