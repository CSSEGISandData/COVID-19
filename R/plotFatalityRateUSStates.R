...
plotCovidFatalityRateUSStates <- function(data,states,yLimits=NULL){
  ...
  aSubset %>% group_by(.,Province_State,date) %>%
    summarise(., Confirmed = sum(Confirmed),
              Deaths = sum(Deaths),
              Case_Fatality_Ratio = Deaths * 100000 / (Confirmed) ) -> df
  df$date <- mdy(df$date)
  asOfDate <- max(df$date)
  ggplot(df, aes(date,Case_Fatality_Ratio, group = Province_State)) + 
    geom_line(aes(group = Province_State), color = "grey80") +
    geom_point(aes(color = Province_State)) + scale_x_date(date_breaks = "2 days") +
