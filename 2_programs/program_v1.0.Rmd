...
###############################################################################
#################### Temporary analysis only for data cleaning ###############
### Temp analysis of cumulative cases #######
data_cases = df_all %>% group_by(country) %>% filter(date_new >= date_first_case) %>% mutate(days_since_first_case = as.numeric(as.character(difftime(date_new,date_first_case, units = 'days' ))))  %>%  ungroup()
countries = as.character(unique(data_cases$country))
####
i = 10
df.temp = data_cases %>% filter(country %in% countries[i])
ggplot(df.temp, aes(x = days_since_first_case, y = Confirmed_cum, group = country, colour = country )) + geom_point() + geom_line()  + theme_bw(base_size = 9) + 
ylab("Daily deaths") +
xlab("Days since first case") + 
scale_y_continuous(limits = c(0, 1200)) 

    
########## Temp analysis of Death ###########################
data_death = df_all %>% group_by(country) %>% filter(date_new >= date_first_death ) %>% mutate(days_since_first_death = as.numeric(as.character(difftime(date_new,date_first_death, units = 'days' )))) %>%  ungroup()
countries = as.character(unique(data_cases$country))

####
i = 217
df.temp = data_death %>% filter(country %in% countries[i])
ggplot(df.temp, aes(x = days_since_first_death, y = Death_cum, group = country, colour = country )) + geom_point() + geom_line()  + theme_bw(base_size = 9) + 
ylab("Daily deaths") +
xlab("Days since first death") + 
scale_y_continuous(limits = c(0, 200)) 


ggplot(df.temp, aes(x = days_since_first_death, y = death_new, group = country, colour = country )) + geom_point() + geom_line()  + theme_bw(base_size = 7) + 
ylab("Daily deaths") +
xlab("Days since first death") + 
scale_y_continuous(limits = c(0, 1200)) + 
