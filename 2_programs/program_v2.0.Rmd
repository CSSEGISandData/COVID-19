...
i = c("US", "UK", "France", "Spain", "Germany", "Italy", "China")
#df.temp = data_death %>% filter(country %in% countries[i])
df.temp = df_death1 %>% filter(country %in% i)

ggplot(df.temp, aes(x = days_since_first_case, y = Confirmed_cum, group = country, colour = country )) + geom_point(size = 0.9) + geom_line()  + theme_bw(base_size = 9) + 
ylab("Daily deaths") +
xlab("Days since first case")
#scale_y_continuous(limits = c(0, 200)) 

    
########## Temp analysis of Death: x axis: days_since_first_death  ###########################
df_death1 = df_all %>% filter(days_since_first_death >=0) %>% ungroup()
df.temp = df_death1
countries = as.character(unique(df.temp$country))
i = c("France", "Spain", "Italy", "China")
#df.temp = data_death %>% filter(country %in% countries[i])
df.temp = df_death1 %>% filter(country %in% i)
p1 = ggplot(df.temp, aes(x = days_since_first_death, y = Death_cum, group = country, colour = country )) + geom_point(size = 0.9) + geom_line()  + theme_bw(base_size = 9) + 
ylab("Daily deaths") +
xlab("Days since first death")
#scale_y_continuous(limits = c(0, 200)) 


########## Temp analysis of Death: x axis: days_since_first_case  ###########################
df_death2 = df_all %>% filter(days_since_first_case >=0) %>% ungroup()
df.temp = df_death2
countries = as.character(unique(df.temp$country))
i = c("France", "Spain", "Italy", "China")
#df.temp = data_death %>% filter(country %in% countries[i])
df.temp = df.temp %>% filter(country %in% i)
