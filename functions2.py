...
# @Last Modified time: 2020-04-04 16:34:28
...
def get_time_columns(columns):
    ...
    for col in columns:
        if col[0].isnumeric():
            time_datetime.append(col)
            time_datetime.append(datetime.datetime.strptime(col, '%m/%d/%y'))
    return time_datetime, time_datetime

## reformat time columns to mm/dd/yy
def reformat_time(df):
    time_datetime, time_datetime = get_time_columns(df.columns)
    times_dic = {}
    for time in time_datetime:
        date = ''
        for s in time.split('/'):
            if(len(s) == 1):
                date += '0' + s + '/'
...
def reformat_time_datetimeing(time):
    date = ''
    for s in time.split('/'):
        if(len(s) == 1):
            date += '0' + s + '/'
...
def reshape_dataframe(df, time_datetime, **kwarg):
    ...
    if('Confirmed' in df.index):
        ...
        l1 = df_new.loc[time_datetime[2:],'Daily_Confirmed'].to_list()
        l2 = df_new.loc[time_datetime[1:-1],'Daily_Confirmed'].to_list()
        l3 = df_new.loc[time_datetime[2:],'Daily_Deaths'].to_list()
        l4 = df_new.loc[time_datetime[1:-1],'Daily_Deaths'].to_list()
        df_new.loc[time_datetime[2:],'GFc'] = np.divide(l1, l2)
        df_new.loc[time_datetime[2:],'GFd'] = np.divide(l3, l4)
        ...
        l1 = df_new.loc[time_datetime[2:],'Daily'].to_list()
        l2 = df_new.loc[time_datetime[1:-1],'Daily'].to_list()
        df_new.loc[time_datetime[2:],'GF'] = np.divide(l1, l2)
...
def get_df_ctry_today(df_confirmed, df_deaths, df_world_population, time_datetime):
    ...
    df_ctry_today['New_Deaths'] = df_deaths.loc[:,time_datetime[-1]] - df_deaths.loc[:,time_datetime[-2]]
...
def get_growth_rate(df_ctry, time_datetime):
    x = np.arange(len(time_datetime))
    y = df_ctry.Confirmed.to_list()

    res = df_ctry[df_ctry.GF_rolling_thr != 0.0].bfill(axis=1).index[0]
    ind_t0 = max(0, time_datetime.index(res)-3)
    t = np.arange(len(time_datetime))[ind_t0:]- ind_t0
    pt = y[ind_t0:]
...
######### plotting #########
### Setting tick labels from a list of values (time_datetime or test_time_datetime)
...
def format_fn_tests(tick_val, tick_pos):
    if(tick_val >= 0 and tick_val < len(test_time_datetime)):
        return test_time_datetime[int(tick_val)]
    else:
        return None
...
def format_fn_future(tick_val, tick_pos):
    if(tick_val >= 0 and tick_val < len(time_datetime)):
        ...
        return f'+{tick_val - len(time_datetime)}'
...
#     legend = []
#     for cat in plotting_params['time_series_cols']:
#         legend.append(f'{cat}: {df_region.loc[time_datetime[-1], cat]:,}')
...
#     ax.legend(legend)
    ...
    x = np.arange(-1, len(time_datetime)+2)
    ...
    ax.set_xlim([-1, len(time_datetime)])
...
#         ind_td = max(0, time_datetime.index(res_d)-3)
        ...
        if(future == 0):
            ...
            for k in [0,1]:
                ...
                if(ind < len(time_datetime)-ind_tc):
                    date_max.append(time_datetime[ind+ind_tc])
                else:
                    date_max.append(f'+{ind-(len(time_datetime)-ind_tc)+1}')
        ...
        if(future == 0):
            ...
            for k in [0,1]:
                if(k == 0):
                    ...
                    ind = int(np.max(ind_midds))
                if(ind+ind_td < len(time_datetime)):
                    date_max.append(time_datetime[ind+ind_td])
                else:
                    date_max.append(f'+{ind-(len(time_datetime)-ind_td)+1}')
        ...
        if(future != 0 and death_to_confirmed):
            ...
            for k in [0,1]:
                if(k == 0):
                    ...
                    ind = int(np.max(ind_midds))
                if(ind < len(time_datetime)-ind_td):
                    date_max.append(time_datetime[ind+ind_td])
                else:
                    date_max.append(f'+{ind-(len(time_datetime)-ind_td)+1}')
            confirmed_total = np.mean(y_ends)
...
#     ax.set_ylim([-0.5, ymax])
    ...
    for ele in plot_list:
        ...
        df_ele = reshape_dataframe(df_ele, time_datetime)
...
def plot_china_prov(df_mc_confirmed, df_mc_deaths, df_hb, df_co, **kwarg):
    ...
    # _ = ax2.set_xlim(right = len(time_datetime))